...
def get_folder_path():
    """
    Meminta pengguna untuk memasukkan path folder dan memvalidasinya.
    Mengembalikan objek Path yang valid.
    """
    while True:
        print("\n" + LOGO_ASCII)
        print("----------------------------------------------------")
        log("📁", "Masukkan path folder yang ingin Anda proses (mis. C:\\Users\\Nama\\Folder):")
        folder_input = input(">> ")
        
        try:
            folder_path = Path(folder_input.strip('"').strip("'"))
            if not folder_path.is_dir():
                log("❌", f"Direktori tidak ditemukan: '{folder_path}'")
                print("Pastikan path yang Anda masukkan benar.")
            else:
                return folder_path
        except (TypeError, OSError) as e:
            log("❌", f"Kesalahan pada jalur folder: {e}")
            print("Pastikan format path benar dan coba lagi.")
...
def main():
    """Fungsi utama untuk menjalankan program dalam loop."""
    while True:
        folder_target = get_folder_path()
        ubah_ekstensi(folder_target, DEFAULT_IMG_EXT, MOVE_TO_VIDEO_FOLDER)
        
        print("----------------------------------------------------")
        print("  ✅ Proses selesai! Pilih opsi berikut:")
        print("  1. Proses folder lain")
        ...
        if pilihan == '1':
            ...
            log("⚠️", "Pilihan tidak valid. Silakan masukkan '1' atau '2'.")
            time.sleep(1)
...
if __name__ == "__main__":
    ...
    print("\n" + LOGO_ASCII)
    print("----------------------------------------------------")
    print("      👋 Terima kasih telah menggunakan skrip ini. Sampai jumpa!")
    print("----------------------------------------------------")